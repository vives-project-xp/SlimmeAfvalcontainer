...
class LedController:
    ...
    def _blink_all_color(self, rgb: tuple[int, int, int], cycles: int = 6, delay: float = 0.18) -> None:
        for _ in range(cycles):
            self._show_all_color(rgb)
            time.sleep(delay)
            self._set_all(OFF)
            time.sleep(delay)
        self._show_all_color(rgb)
        ...
        with self._lock:
            try:
                ...
                if cmd.startswith("confirm_"):
                    # Flash all strips orange 2x to signal successful throw-in, then idle.
                    CONFIRM_COLOR = (255, 120, 0)
                    for _ in range(2):
                        self._show_all_color(CONFIRM_COLOR)
                        time.sleep(0.3)
                        self._set_all(OFF)
                        time.sleep(0.2)
                    self._show_all_bins()
                ...
                if cmd == "reject_blink":
                    self._blink_all_color(RESULT_RED)
                    self.current_choice = Choice.NONE
                    return "OK: REJECT_BLINK"
